...
class MainView(tk.Tk):
    ...
    def _create_frames(self):
        ...
        # Preprocessing frame
        self.frames['preprocessing'] = PreprocessingFrame(self.container, self.controller)
        self.frames['preprocessing'].bind("<<NextStep>>", self.on_preprocessing_next)
        
        # Learning Type frame - NOUVEAU
        self.frames['learning_type'] = LearningTypeFrame(self.container, self.controller)
        self.frames['learning_type'].bind("<<NextStep>>", self.on_learning_type_next)
        self.frames['learning_type'].bind("<<PreviousStep>>", self.on_learning_type_back)
        
        # Clustering Metrics frame - NOUVEAU
        self.frames['clustering_metrics'] = ClusteringMetricsFrame(self.container, self.controller)
        self.frames['clustering_metrics'].bind("<<NextStep>>", self.on_clustering_metrics_next)
        self.frames['clustering_metrics'].bind("<<PreviousStep>>", self.on_clustering_metrics_back)
    ...
    def show_current_step(self):
        ...
        current_step_mapping = step_mapping.get(learning_type, step_mapping["unsupervised"])
        ...
        if frame_name == 'algorithms' and 'algorithms' not in self.frames:
          self.frames['algorithms'] = AlgorithmsFrame(self.container, self.controller)
          self.frames['algorithms'].bind("<<NextStep>>", self.on_algorithms_next)
